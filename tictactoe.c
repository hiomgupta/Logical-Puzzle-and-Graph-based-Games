...
int has_won(board_t board, player_t player) {
    for (int row = 0; row < BOARD_SIZE; ++row) {
        ...
        return 1; // If the entire row is the same as `player`, player has won.
    next_row:
        ;
    }
    ...
    for (int col = 0; col < BOARD_SIZE; ++col) {
        ...
        return 1; // If the entire column is the same as `player`, player has won.
    next_col:
        ;
    }
    ...
    return 1; // If the main diagonal is the same as `player`, player has won.

next_diagonal:
    for (int i = 0; i < BOARD_SIZE; ++i) {
        if (board[i][BOARD_SIZE-1-i] != player) return 0;
    }
    return 1; // If the anti-diagonal is the same as `player`, player has won.
}
